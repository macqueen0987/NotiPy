...
async def get_notion_database(
    ...
    if notionlink:
        return notionlink
    query = select(NotionDatabase).where(
...
async def get_linked_notion_database(
    ...
    """
    query = select(NotionDatabase).where(
        NotionDatabase.channel_id == channelid)
    result = await conn.execute(query)
    notionlink = result.scalars().first()
    if notionlink is None:
        return None
...
async def link_notion_database(
    conn: AsyncSession, discordid: int, databaseid: str, channelid: int
) -> None:
    """
    ...
    notionlink = NotionDatabase(
        server_id=discordid, database_id=databaseid, channel_id=channelid
    )
...
async def get_all_updated_pages(
    conn: AsyncSession,
) -> Sequence[Row[tuple[NotionPages, int, str]]]:
    """
