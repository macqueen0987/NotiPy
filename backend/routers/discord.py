...
from common import *
from fastapi import (APIRouter, BackgroundTasks, Depends, Request, Response, status, Body, HTTPException)
...
@router.get("/{serverid}")
@checkInternalServer
async def get_server(request: Request, serverid: int, eager_load: int = 0, conn=Depends(get_db)):
    """
...
@router.post("/clean")
@checkInternalServer
async def add_server(request: Request, serverids: list[int] = Body(...), conn=Depends(get_db)):
    """
...
async def set_mod_role(
    request: Request, serverid: int, roleid: int = Body(...), conn=Depends(get_db)
):
    """
...
async def set_webhook_channel(
    request: Request, serverid: int, channelid: int = Body(...), conn=Depends(get_db)
):
    """
...
@checkInternalServer
async def get_notion_database(request: Request, serverid: int, conn=Depends(get_db)):
    """
...
@checkInternalServer
async def set_token(request: Request, serverid: int, token: str = Body(None), conn=Depends(get_db)):
    """
...
@checkInternalServer
async def get_notion_tag(request: Request, serverid: int, conn=Depends(get_db)):
    """
...
async def add_notion_tag(
    ...
    if len(current_tags) >= 3:
        raise HTTPException(status_code=429, detail="You can assign up to 3 Notion tags per server.")
    # 태그 추가
