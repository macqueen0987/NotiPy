...
def validate_project_field(field: str, value: str) -> Union[str, int, list[str], None]:
    """
...
async def editproject(ctx: ComponentContext, paginator: Paginator, _):
    ...
    msg = await ctx.send(_("select_edit_field_prompt"), components=[select_menu], ephemeral=True)
    res = await wait_for_component_interaction(ctx, select_menu, msg, timeout=60)
    if not res:
        return None
    select_ctx, field = res
    parsed = None
    if field in {"category", "team_size", "core_features", "tech_stack", "complexity", "database", "platform"}:
        # Modal 요청
        modal = Modal(
        ...
        modalctx = select_ctx
        await modalctx.defer(edit_origin=True)
    status, response = await apirequest(f"/llm/projects/{userid}/{projectid}", method="PUT", json={field: parsed})
    if status == 204:
        return await modalctx.send(_(f"project_not_found"), ephemeral=True)
    if status != 200:
        raise ValueError(f"Error in /llm/projects/{userid}/{projectid}")
    await modalctx.send(_(f"{field}_update_success"), ephemeral=True, components=[], delete_after=10)
    return None

async def send_project_paginator(ctx: SlashContext, _, content: str = "", callback: callable = None):
    """
    ...
    """
    status, response = await apirequest(f"/llm/projects/{ctx.author_id}/server/{ctx.guild_id}")
    if status == 204:
    ...
    paginator.callback = partial(callback, paginator=paginator, _=_) if callback else None
...
def create_project_embed(project, _):
    ...
    embed = Embed(title=project["name"], description=project["description"], color=createRandomColor())
    embed.add_field(name=_("core_features"), value=", ".join(project["core_features"]), inline=False)
    embed.add_field(name=_("tech_stack"), value=", ".join(project["tech_stack"]), inline=False)
    embed.add_field(name=_("project_id"), value=project["project_id"], inline=True)
    embed.add_field(name=_("category"), value=project["category"], inline=True)
    embed.add_field(name=_("team_size"), value=project["team_size"], inline=True)
    embed.add_field(name=_("complexity"), value=project["complexity"], inline=True)
    ...
    embed.add_field(name=_("notifications"), value=project["notifications"], inline=True)
    embed.add_field(name=_("map_integration"), value=project["map_integration"], inline=True)
    embed.add_field(name=_("auth_required"), value=project["auth_required"], inline=True)
    return embed
...
async def manage_project_member(ctx: ComponentContext, paginator: Paginator, _):
    """
    ...
    projectid, projectname = get_project_from_paginator(paginator)
    options = []
    options.append(StringSelectOption(label=_("view_project_member"), value=f"view_project_member"))
    options.append(StringSelectOption(label=_("add_project_member"), value=f"add_project_member"))
    options.append(StringSelectOption(label=_("remove_project_member"), value=f"remove_project_member"))
    component = StringSelectMenu(*options, placeholder=_("select_project_member_action"), custom_id=f"manage_project_member_select_{projectid}")
    await ctx.send(content=_("select_project_member_action_desc").format(projectname), components=[component], ephemeral=True)
...
async def set_project_member(ctx: ComponentContext, paginator: Paginator, _):
    ...
    # FastAPI API 호출
    status, response = await apirequest(f"/llm/project/{ctx.author.id}/{projectid}/assign/auto", method="POST")
    if status == 204:
        return await ctx.send(_("project_not_found_or_no_roles_or_members"), ephemeral=True)
    elif status != 200:
        raise ValueError(f"Error in /llm/project/{ctx.author.id}/{projectid}/assign/auto")
    data = response.get("data", {})
    if not data:
        return await ctx.send(_("no_assignments_result"), ephemeral=True)
    embed = Embed(title=_("role_assignment_result_title"), color=createRandomColor())
    for rolename, discord_ids in data.items():
        mentions = ", ".join(f"<@{uid}>" for uid in discord_ids)
        embed.add_field(name=rolename, value=mentions or "(none)", inline=False)
    return await ctx.send(_("project_members_assigned_success"), embed=embed, ephemeral=True)
...
class Projects(Extension):
    ...
    @projectBase.subcommand(sub_cmd_name=getname("list"), sub_cmd_description=getdesc("projects_list"))
    @check(is_moderator)
    @localize()
    ...
    @projectBase.subcommand(sub_cmd_name=getname("create"), sub_cmd_description=getdesc("projects_create"))
    ...
    async def create_project(self, ctx: SlashContext, _):
        ...
        if not ctx.guild:
            ...
            return
        modal = Modal(ShortText(label=_("project_create_title"), value=_("project_create_title"), max_length=255, custom_id="title"),
                      ParagraphText(label=_("project_create_description"), value=_("project_create_description"), custom_id="description"),
        ...
        status, response = await apirequest(f"/llm/projects/{ctx.author_id}", method="POST", json=data)
        if status != 200:
            raise ValueError(f"Error in /llm/projects/{ctx.author_id}")
        proj = response['data']
    ...
    @projectBase.subcommand(sub_cmd_name=getname("remove"), sub_cmd_description=getdesc("projects_remove"))
    @check(is_moderator)
    @localize()
    async def remove_project(self, ctx: SlashContext, _):
        ...
        if not response:
            await ctx.send(_("no_projects_found"), ephemeral=True)
            return
        options = [StringSelectOption(label=project["name"], value=project["project_id"]) for project in response['projects']]
        select = StringSelectMenu(
            *options,
            placeholder=_("select_project_to_remove"),
            custom_id="remove_project_select"
        )
        await ctx.send(_("select_project_to_remove_desc"), components=[select], ephemeral=True)
    ...
    async def remove_project_select(self, ctx: ComponentContext, _):
        ...
        status, response = await apirequest(f"/llm/projects/{ctx.author_id}/{project_id}", method="DELETE")
        if status != 200:
            raise ValueError(f"Error in /llm/projects/{ctx.author_id}/{project_id}")
        await ctx.edit_origin(content=_("project_removed_success"), components=[])

    @projectMemberGroup.subcommand(sub_cmd_name=getname("manage"), sub_cmd_description=getdesc("manage_project_member"))
    @check(is_moderator)
    @localize()
    async def add_project_member(self, ctx: SlashContext, _):
        ...
        """
        await send_project_paginator(ctx, _, "select_project_to_manage_member", manage_project_member)
    ...
    async def manage_project_member_callback(self, ctx: ComponentContext, _):
        ...
        if action in ["remove_project_member", "view_project_member"]:
            status, response = await apirequest(f"/llm/project/{ctx.author.id}/{projectid}/member")
            if status == 204:
                return await ctx.send(_("project_not_found_or_no_member"), ephemeral=True)
            if status != 200:
                raise ValueError(f"Error in /llm/project/{ctx.author.id}/{projectid}/member")
            members = response['members']

        if action == "add_project_member":
            component = UserSelectMenu(placeholder=_("select_member_to_add"), custom_id=f"add_project_member_{projectid}")
            await ctx.send(_("select_member_to_add_desc"), components=[component], ephemeral=True)
            ...
            for member in members:
                ...
                options.append(StringSelectOption(label=mem.display_name, value=str(githubid)))
            component = StringSelectMenu(*[options], placeholder=_("select_member_to_remove"), custom_id=f"remove_project_member_{projectid}")
            await ctx.send(_("select_member_to_remove_desc"), components=[component], ephemeral=True)
        elif action == "view_project_member":
            embed = Embed(title=_("project_members_list"), color=createRandomColor())
            for member in members:
                discord_id = member['discordid']
                mem = await ctx.guild.fetch_member(discord_id)
                if not mem:
                    continue
                embed.add_field(name=mem.display_name, value=f"GitHub ID: {member['githubid']}", inline=False)
    ...
    async def add_project_member_callback(self, ctx: ComponentContext, _):
        ...
        status, response = await apirequest(f"/llm/project/{ctx.author.id}/{projectid}/member", method="POST", json=member.id)
        if status == 204:
    ...
    async def remove_project_member_callback(self, ctx: ComponentContext, _):
        ...
        status, response = await apirequest(f"/llm/project/{ctx.author.id}/{projectid}/member/{githubid}", method="DELETE")
        if status == 204:
            return await ctx.send(_("project_not_found_or_no_member"), ephemeral=True)
        if status != 200:
            raise ValueError(f"Error in /llm/project/{ctx.author.id}/{projectid}/member/{githubid}")
        return await ctx.send(_("project_member_removed"), ephemeral=True, delete_after=5)

    @projectMemberGroup.subcommand(sub_cmd_name=getname("set"), sub_cmd_description=getdesc("set_project_member"))
    @check(is_moderator)
    @localize()
    async def set_project_member_trigger(self, ctx: SlashContext, _):
        ...
        """
        await send_project_paginator(ctx, _, "select_project_to_set_member", callback=set_project_member)
