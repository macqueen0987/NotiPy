...
from commons import (MyFunctions, devserver, getdesc, getname, localize,
                     makerequest, notion_token)
from fastapi import Request
from interactions import (Client, Embed, Extension, OptionType, SlashContext,
...
async def notionWebhook(bot: Client, params: dict) -> None:
    ...
    if "verification_token" in params:
        await channel.send(
            "Here is your verification token: " + params["verification_token"]
        )
        return
    ...
    status, response = await makerequest(
        f"https://api.notion.com/v1/users/{authorid}", "GET", headers=header
    )
    if status != 200:
        return
    ...
    embed = Embed(
        title=type_, description="Workspace: " + workspacename, color=0x00FF00
    )
