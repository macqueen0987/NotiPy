...
from interactions import (Extension, SlashContext, Client, slash_command, slash_option, OptionType, SlashCommand,
                          Button, ButtonStyle, component_callback, ComponentContext)
from commons import devserver, localize, getname, getdesc, apirequest, githuburl
import asyncio
import os

oauth2_url = os.environ['DISCORD_OAUTH2_URL']

githubBase = SlashCommand(name=getname("github"), description=getdesc("github"))
...
class User(Extension):
    ...
    @githubBase.subcommand(sub_cmd_name=getname("gitlink"), sub_cmd_description=getdesc("gitlink"))
    @localize()
    async def gitlink(self, ctx: SlashContext, _):
        ...
        gitbuttons = [
            Button(style=ButtonStyle.URL, label=_("link_github_button"), url=oauth2_url),
            Button(style=ButtonStyle.PRIMARY, label=_("check_github_linked_button"), custom_id="check_github_linked")
        ]
    ...
    async def check_github_linked(self, ctx: ComponentContext, _):
        ...
        github = response['github']
        mainurl = githuburl + "/" + github['github_login']
        await ctx.edit_origin(content=_("success_linked_github") + "\n" + mainurl, components=[])
